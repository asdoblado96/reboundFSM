...
static int fsm_rebound_check(fsm_t* f)
{
...
static int fsm_rebound_is_timeout(fsm_t* f)
{
    fsm_rebound_t* fp = (fsm_rebound_t*)f;
    return (timer_get_tick() >= fp->next_timeout);
}

static void fsm_rebound_set_timeout(fsm_t* f)
{
    fsm_rebound_t* fp = (fsm_rebound_t*)f;
    fp->next_timeout = timer_get_tick() + fp->time_timeout;
}

static void fsm_rebound_reset(fsm_t* f)
{
    fsm_rebound_t* fp = (fsm_rebound_t*)f;
    if (fp->reset) {
        fp->reset();
